		...
		<div class="flex flex-col items-center w-full">
			<div class="flex w-full px-2 rounded-md">
				<div class="w-full">
					<ul class="w-full text-sm font-medium text-gray-900 bg-white rounded-lg">
						<li v-for="job in jobs" :key="job.information" class="w-full border border-gray-200 rounded-lg"
							style="margin-bottom: 10px;" :class="{ 'bg-sky-400': jobId === job.jobId }">
							<div class="flex items-center px-3 pr">
								...
								<label :for="'list-radio-license-' + job.jobId"
									...
									:class="{ 'text-white': jobId === job.jobId }">{{
										`${job.jobName} (${job.jobNameId})` }}</label>
							</div>
						</li>
					</ul>
			...
			<div class="flex">
				<button @click="selectJob"
					class="rounded-md bg-white px-3 py-2 text-xl transition duration-300 ease-in-out hover:bg-sky-400 hover:text-white">
					Start training
				</button>
			</div>
		</div>
					...
					<TransitionChild as="template" enter="ease-out duration-300"
						...
						<DialogPanel class="fixed inset-0 flex items-center justify-center bg-opacity-75 bg-gray-500 z-50">
							<div class="flex flex-row items-center">
								<svg class="animate-spin -ml-1 mr-3 w-12 text-white" xmlns="http://www.w3.org/2000/svg"
									fill="none" viewBox="0 0 24 24">
									<circle class="opacity-25" cx="12" cy="12" r="10" stroke="rgb(56 189 248)"
